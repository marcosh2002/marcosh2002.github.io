<?php
/**
 * Fixed Header.php Updater - Correct WordPress Structure
 * Upload this file to your server, run it once, then delete it
 * 
 * USAGE:
 * 1. Upload this file to: royaldesicrew.com/upload-header-fixed.php
 * 2. Visit: royaldesicrew.com/upload-header-fixed.php?confirm=yes
 * 3. Check your website - CSS should now display, no duplicate content
 * 4. DELETE both upload scripts from your server
 */

// Verify confirmation parameter (safety check)
if (!isset($_GET['confirm']) || $_GET['confirm'] !== 'yes') {
    die('<h1>Corrected CSS Upload Script</h1><p>This will fix the duplicate content issue and update header.php properly.</p><p><strong>WARNING:</strong> This file should be deleted after running!</p><p><a href="?confirm=yes">Click here to proceed</a></p>');
}

// Define the WordPress path
$wp_root = dirname(__FILE__);
$header_file = $wp_root . '/wp-content/themes/royal-desi-crew-theme/header.php';
$footer_file = $wp_root . '/wp-content/themes/royal-desi-crew-theme/footer.php';

// Check if files exist
if (!file_exists($header_file)) {
    die('<p style="color: red;"><strong>ERROR:</strong> header.php not found at ' . $header_file . '</p>');
}

// CORRECTED header.php - Only head section and opening tags, NO page content
$new_header_content = <<<'EOT'
<?php
/**
 * Header Template
 * 
...
.badge-wrapper {
    ...
    border-radius: 65px;
    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.6), inset 0 1px 0 rgba(212, 175, 55, 0.15);
}
...
// Corrected footer.php (much simpler - just closes tags)
$new_footer_content = <<<'EOT'
<?php
/**
 * Footer Template
 * 
...
// Write header.php
$header_result = file_put_contents($header_file, $new_header_content);

// Write footer.php
$footer_result = file_put_contents($footer_file, $new_footer_content);

if ($header_result !== false && $footer_result !== false) {
    echo '<h2 style="color: green;">✓ SUCCESS!</h2>';
    echo '<p><strong>Both files updated:</strong></p>';
    echo '<ul>';
    echo '<li>✓ header.php - Fixed with hardcoded CSS + proper header only</li>';
    echo '<li>✓ footer.php - Fixed with correct closing tags</li>';
    echo '</ul>';
    echo '<p><strong>Next steps:</strong></p>';
    echo '<ol>';
    echo '<li>Visit <a href="https://royaldesicrew.com" target="_blank">https://royaldesicrew.com</a></li>';
    echo '<li>Press <strong>Ctrl+Shift+Delete</strong> for hard refresh</li>';
    echo '<li>Your landing page should now display correctly with full styling - no duplicate content!</li>';
    echo '</ol>';
    echo '<p style="color: red; font-weight: bold; margin-top: 20px;">⚠️ IMPORTANT: Delete upload scripts!</p>';
    echo '<p style="background: #f0f0f0; padding: 10px; border-radius: 5px;">';
    echo 'Delete these files from cPanel:<br>';
    echo '- <code>upload-header.php</code> (old one)<br>';
    echo '- <code>upload-header-fixed.php</code> (this one)';
    echo '</p>';
} else {
    echo '<p style="color: red;"><strong>ERROR:</strong> Could not write files</p>';
    if ($header_result === false) {
        echo '<p>Failed to write header.php</p>';
    }
    if ($footer_result === false) {
        echo '<p>Failed to write footer.php</p>';
    }
}
